...
export default function BookedRides() {
  ...
  const fetchRides = useCallback(async (page = currentPage, q = query) => {
    try {
      setLoading(true);
      const res = await api.get(`/rides?status=booked&page=${page}&limit=20&search=${encodeURIComponent(q)}`);
    ...
    <div className="ride-page-wrap">
      <div className="ride-card">
        <div className="ride-card-header">
          <h4 className="ride-card-title">Booked Rides</h4>
        </div>
                  ...
                  rides.map((r, i) => (
                    <tr key={r.id || i}>
                      ...
                      <td style={{ textAlign: "center" }}>
                        <span className="ride-badge ride-badge-booked">Booked</span>
                      </td>
                      <td>
