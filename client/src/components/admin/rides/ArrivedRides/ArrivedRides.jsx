...
export default function ArrivedRides() {
  ...
  const fetchRides = useCallback(async (page = currentPage, q = query) => {
    try {
      setLoading(true);
      const res = await api.get(`/rides?status=arrived&page=${page}&limit=20&search=${encodeURIComponent(q)}`);
      ...
      console.error("Error fetching arrived rides:", err);
    } finally {
      setLoading(false);
    }
  }, [currentPage, query]);
    ...
    <div className="ride-page-wrap">
      <div className="ride-card">
        <div className="ride-card-header">
          <h4 className="ride-card-title">Arrived Rides</h4>
        </div>
          ...
          <div className="ride-table-responsive">
            <table className="ride-table">
              ...
              <tbody>
                {loading ? (
                  <tr>
                    <td colSpan={10} className="ride-no-data">
                      Loading arrived rides...
                    </td>
                  </tr>
                ) : rides.length === 0 ? (
                  <tr>
                  ...
                  rides.map((r, i) => (
                    <tr key={r.id || i}>
                      ...
                      <td style={{ textAlign: "center" }}>
                        <span className="ride-badge ride-badge-arrived">Arrived</span>
                      </td>
                      <td>
