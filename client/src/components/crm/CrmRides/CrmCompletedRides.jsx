...
export default function CrmCompletedRides() {
  ...
  const fetchRides = useCallback(async (q = query) => {
    try {
      setLoading(true);
      const res = await api.get(`/rides?status=completed&search=${encodeURIComponent(q)}`);
      if (res.data && res.data.success) {
        setRides(res.data.rides || []);
      }
    } catch (err) {
      console.error("Error fetching CRM completed rides:", err);
    } finally {
      setLoading(false);
    }
  }, [query]);
